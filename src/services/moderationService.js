  ...
  static async banUser({
    ...
    try {
      ...
      if (targetMember) {
        ...
        if (!botCheck.valid) {
          throw new AquaticSMPHelperError(botCheck.error, ErrorTypes.PERMISSION, botCheck.error);
        }

        const modCheck = this.validateHierarchy(moderator, targetMember, 'ban');
        if (!modCheck.valid) {
          throw new AquaticSMPHelperError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
        }
      } else {
